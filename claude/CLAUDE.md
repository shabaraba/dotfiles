...
- **BREAKING CHANGEは慎重に使用する**: ユーザーに実質的な影響を与える破壊的変更の場合のみ使用
- **使用すべき場合**:
  - APIの互換性を破る変更（関数シグネチャ、引数、戻り値の変更）
  - 設定ファイルのフォーマット変更でユーザーの手動対応が必要
  - コマンドラインインターフェースの変更
  - 削除された機能で代替手段が必要
- **使用すべきでない場合**:
  - 内部実装の変更（ユーザーから見た動作は同じ）
  - ドキュメントやコメントの更新
  - リファクタリング（外部インターフェースは不変）
  - 非公開APIや内部関数の変更
  - バグ修正（元々想定された動作への修正）
